...
class Dice
{
     ...
     * @param array  $rule The container can be fully configured using rules provided by associative arrays. See {@link
     *                     https://r.je/dice.html#example3} for a description of the rules.
     */
     ...
     * @param array  $share Whether or not this class instance be shared, so that the same instance is passed around
     *                      each time
     *
    ...
    private function getClosure($name, array $rule)
    {
        ...
        if (isset($rule['shareInstances'])) {
            $closure = function (array $args, array $share) use ($closure, $rule) {
                return $closure(
                    $args,
                    array_merge($args, $share, array_map([$this, 'create'], $rule['shareInstances']))
                );
            };
        }
    ...
    /**
     * Looks for 'instance' array keys in $param and when found returns an object based on the value see {@link
     * https:// r.je/dice.html#example3-1}
     *
     ...
     *
     * @param \ReflectionMethod $method An instance of ReflectionMethod (see: {@link http://
     *                                  php.net/manual/en/class.reflectionmethod.php})
    ...
    private function getParams(\ReflectionMethod $method, array $rule)
    {
        ...
        return function (array $args, array $share = []) use ($paramInfo, $rule) {
            ...
            foreach ($paramInfo as list($class, $param, $sub)) {
                ...
                if ($class) {
                    $parameters[] = $sub ? $this->expand($rule['substitutions'][$class], $share, true) : $this->create(
                        $class,
                        [],
                        $share
                    );
